...
public class IntDoubleWrapper {
	...
	public static void main(String[] args) {
		...
		// 2. String -----> rlqhsgud
		// (!) parseType() : 각 포장클래스의 static 메소드 사용
		String intFrm = String.format("문자열[%s]==>기본형(int)[%d]", intInput, Integer.parseInt(intInput));
		String dbFrm = String.format("문자열[%s]==>기본형(double)[%.3f]", dbInput, Double.parseDouble(dbInput));
		
		// (2) 변경된 값들 출력
		System.out.println(intFrm);
		System.out.println(dbFrm);
		
		// static import 를 진행하면 partesType() 메소드 호출시 
		// 클래스 이름 생략
		intFrm = String.format("문자열[%s]==>기본형(int)[%d]", intInput, parseInt(intInput));
		dbFrm = String.format("문자열[%s]==>기본형(double)[%.3f]", dbInput, parseDouble(dbInput));
		...
		dbFrm = String.format("문자열[%s]==>기본형(double)[%.3f]", dbInput, new Double(dbInput).doubleValue());
		...
		// String 변경 확인을 위하여 String 
		tenStr += "입니다.";
		sixtyStr += "입니다";
		
		// 문자열 접한 연산결과 String 변수 출력
		...
		//=====================================================
		// 5. 포장클래스의 aotounBoxing, aotoBoxing
		// autoUnboxing : 참조형 인스턴스가 기본형 변수에 
		// 				  변환 메소드 호출없이 바로 담기는 기법
